...
func (repo *RoleplayRepository) GetByToken(token string) (*[]entity.RoleplayMySql, error) {
	db := util.GetMySQL()
	query := fmt.Sprintf("SELECT token, tools_type, instruction_file, instruction, meeting_url, participant_instruction, start_date, end_date, assessor_1_id, assessor_2_id, assessor_3_id, assessor_4_id, asesi, role_player_id FROM %s WHERE token = '%s'", repo.tableName, token)
	log.Println(query)
	results, err := db.Query(query)
	if err != nil {
		return nil, err
